...
//cv::Mat gbr转avFrame yuv420
void cvMatToAVFrame(cv::Mat& mat, AVFrame*& frame, int64_t t)
{
	if (!frame) {
		//printf("进入!frame了！！\n");
	...
	if (frame && !mat.empty()) {
		...
		int s = frame_size * 0.25;
		memcpy(frame->data[0], pdata, frame_size);
		memcpy(frame->data[1], pdata + frame_size, frame_size * 0.25);
		memcpy(frame->data[2], pdata + frame_size + s, frame_size * 0.25);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if (ret < 0)
	{
		printf("Open avcodec is faild! ret = %d\n", ret);
		return 0;
	}
	...
	if (!(avFormCtx_Out->oformat->flags & AVFMT_NOFILE))
	{
		...
		if (ret < 0)
		{
			printf("Open file is faild! ret = %d\n", ret);
			return 0;
		}
	}
...
//change
		...
		//frame->pkt_duration = 0;
		//frame->pkt_pos = -1;
		int64_t t = av_rescale_q_rnd(frameNumber, avCodecCtx_Out->time_base, avStream->time_base, (AVRounding)(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
		/*
		将BYTE* 数据存储到 cv::Mat中
		再从cv::Mat 转到 AVFrame 
		*/
		//printf("frameimage length = %d", sizeof(frameimage));


		cv::Mat mat;
		mat = ccs->CaptureImage1();

		//mat.create(cv::Size(1920,1080), CV_8UC3);
		//memcpy(frameimage, mat.data, 1080*1920*3);

		//GetBitmapBits(*(HBITMAP*)frameimage, 1920 * 1080 * 3, mat.data);
		cv::imwrite("123.bmp", mat);
		//printf("rows : %d      cols : %d\n", mat.rows, mat.cols);
		cvMatToAVFrame(mat, frame, t);
